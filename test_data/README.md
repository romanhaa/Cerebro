...
In the manuscript, we used a scRNA-seq dataset containing about 10k human PBMCs from a healthy donor (`pbmc_10k_v3`), generated using the v3 chemistry and processed using Cell Ranger 3.0.0.
...
Briefly, in the example we followed the basic Seurat workflow:
...
* Perform dimensional reduction: tSNE, UMAP, diffusion map, PHATE (`RunTSNE`, `RunUMAP`, `RunDiffusion`, `RunPHATE`).
...
* Export a `.cbr` file that can be loaded into Cerebro (`exportFromSeurat()`).

To test Cerebro, download the `.cbr` file and load it into Cerebro.

## How to reproduce

I suggest to run the example R script in a container using Singularity (here I used Singularity 2.6.0).

```
git clone romanhaa/cerebroManuscript
cd cerebroManuscript
...
This will reproduce the `.cbr` file used in the manuscript.
